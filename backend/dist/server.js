...
import fs from 'fs';
// Import Zod schemas
import { createUserInputSchema, updatePropertyInputSchema, createBookingInputSchema, updateBookingInputSchema, createReviewInputSchema, createMessageInputSchema, updateMessageInputSchema } from './schema.js';
dotenv.config();
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const result = await db.query('INSERT INTO users (user_id, email, password_hash, name, created_at) VALUES ($1, $2, $3, $4, $5) RETURNING user_id, email, name, created_at', [user_id, email.toLowerCase().trim(), password, name.trim(), created_at]);
        const user = result.rows[0];
        // Generate JWT token
        const token = jwt.sign({ user_id: user.user_id, email: user.email }, JWT_SECRET, { expiresIn: '7d' });
        // Create auth token record
        const token_id = uuidv4();
        await db.query('INSERT INTO auth_tokens (token_id, user_id, token, is_valid, created_at) VALUES ($1, $2, $3, $4, $5)', [token_id, user.user_id, token, true, created_at]);
        res.status(200).json({
            user: {
                id: user.user_id,
...
app.get('/api/properties', async (req, res) => {
    try {
        const { location, price_min, price_max, accommodation_type, amenities, limit = '10', offset = '0' } = req.query;
        ...
        if (price_min !== undefined && price_min !== '') {
            const priceMinNum = parseFloat(price_min);
...
app.post('/api/bookings', authenticateToken, async (req, res) => {
    try {
        ...
        const conflictCheck = await db.query('SELECT booking_id FROM bookings WHERE property_id = $1 AND ((start_date <= $2 AND end_date >= $2) OR (start_date <= $3 AND end_date >= $3) OR (start_date >= $2 AND end_date <= $3))', [property_id, start_date, end_date]);
        ...
        const result = await db.query('INSERT INTO bookings (booking_id, property_id, user_id, start_date, end_date, guests, total_price, is_paid, payment_error_message) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *', [booking_id, property_id, user_id, start_date, end_date, guests, total_price, is_paid, payment_error_message]);
        res.json(result.rows[0]);
        // Emit real-time event
...
app.post('/api/reviews', authenticateToken, async (req, res) => {
    try {
        ...
        const bookingCheck = await db.query('SELECT booking_id FROM bookings WHERE property_id = $1 AND user_id = $2 AND end_date < NOW()', [property_id, user_id]);
        if (bookingCheck.rows.length === 0) {
        ...
        const existingReview = await db.query('SELECT review_id FROM reviews WHERE property_id = $1 AND user_id = $2', [property_id, user_id]);
        ...
        const created_at = new Date().toISOString();
        const result = await db.query('INSERT INTO reviews (review_id, property_id, user_id, rating, comment, created_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [review_id, property_id, user_id, rating, comment, created_at]);
        res.json(result.rows[0]);
        // Emit real-time event
...
app.post('/api/messages', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await db.query('INSERT INTO messages (message_id, sender_id, recipient_id, property_id, content, timestamp) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [message_id, sender_id, recipient_id, property_id, content, timestamp]);
        res.json(result.rows[0]);
...
app.patch('/api/messages/:message_id', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await db.query('UPDATE messages SET content = $1 WHERE message_id = $2 RETURNING *', [content, message_id]);
        res.json(result.rows[0]);
        // Emit real-time event
    ...
    */
    socket.on('send_message', async (data) => {
        try {
            ...
            const message_id = uuidv4();
            const timestamp = new Date().toISOString();
            const result = await db.query('INSERT INTO messages (message_id, sender_id, recipient_id, property_id, content, timestamp) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [message_id, socket.user.user_id, recipient_id, property_id, content, timestamp]);
...
*/
app.get('/api/hosts/:host_id/earnings', authenticateToken, async (req, res) => {
    try {
...
*/
app.get('/api/hosts/:host_id/calendar', authenticateToken, async (req, res) => {
    try {
        const { host_id } = req.params;
        // Verify user is requesting their own data
        if (host_id !== req.user.user_id) {
